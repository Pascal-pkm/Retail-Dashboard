...
- **Newsletter** über Resend (Free Tier) — rein privat an die in `NEWSLETTER_TO` hinterlegte(n) eigene(n) Adresse(n), ausgelöst am Ende jedes Cron-Laufs sowie optional beim manuellen Hystreet-Lauf; nur wenn Repo-Variable `NEWSLETTER_ENABLED=true`.
...
4. **Secrets** (Settings → Secrets and variables → Actions → Secrets):
   ...
   - `NEWSLETTER_TO` – Empfänger, kommagetrennt
...
5. **Variablen** (… → Variables):
   ...
   - `NEWSLETTER_FROM` = z. B. `Retail KPI <onboarding@resend.dev>` (eigene Domain bei Resend verifizieren für eigene Absenderadresse)
...
python scripts/newsletter.py daily      # schreibt newsletter_daily_preview.html; versendet nur mit RESEND_API_KEY
```
...
scripts/newsletter.py       HTML-Newsletter + Resend-Versand (privat)
