...
- **Newsletter** über Gmail SMTP (App-Passwort) — rein privat an die in `NEWSLETTER_TO` hinterlegte(n) eigene(n) Adresse(n), ausgelöst am Ende jedes Cron-Laufs sowie optional beim manuellen Hystreet-Lauf; nur wenn Repo-Variable `NEWSLETTER_ENABLED=true`. Design (`scripts/newsletter.py`, seit 07/2026): Farbschema der Website, Spark-Card-Stil angelehnt an Apollos "Daily Spark", Inhalt je Frequenz gescoped (täglich/wöchentlich: Google-Trends + Frequenzen; monatlich zusätzlich ifo-Indizes; quartalsweise zusätzlich Zalando-IR-Kennzahlen).
...
| täglich | Passantenfrequenzen – 6 Städte (Oldenburg, Würzburg, Dortmund, Neuss, Bamberg, Augsburg) | Region + Standort (eigener Tab) | echte Fußgängerzahlen; 4 Städte lizenzrechtlich unkritisch, Bamberg/Augsburg mit dokumentiertem Restrisiko – siehe COMPLIANCE.md Abschnitt 6 |
