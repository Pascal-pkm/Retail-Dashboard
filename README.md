...
- **Periodenübersicht ("Auf einen Blick")**: jeder Frequenz-Reiter (Täglich/Wöchentlich/Monatlich/Quartal) zeigt oben 3 Kennzahlen (Google-Trends-Modeindex, Passantenfrequenz gesamt, Radverkehr gesamt) sowie eine dynamische Top-3-Abweicher-Liste (Frequenzen + Google Trends), jeweils mit der für die Frequenz passenden Abweichung (Tag/Woche/Monat/Quartal ggü. Vorjahr bei Rad-/Fußverkehr wegen Saisonalität, ggü. Vorperiode bei Google Trends). Ein noch laufender Bucket (heutiger Tag bzw. eine Region mit Meldeverzug) wird dabei automatisch übersprungen, damit keine Schein-Einbrüche durch unvollständige Daten entstehen (siehe `bucketRegionSummary`/`regionMoverChange`/`dropIncompleteBucket` in `docs/index.html`).
